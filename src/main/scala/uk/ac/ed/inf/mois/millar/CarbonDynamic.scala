...
class CarbonDynamic extends Process with VarCalc{
  ...
  val GasConstant = Double("c:GasConstant") default(8.314) param()
  GasConstant annotate("description", "Gas constant")
  GasConstant annotate("units", "J/Kmol")
  ...
  val A_c = Double("c:A_c") nonnegative()
  ...
  val J = Double("c:J") default(0) nonnegative()
  J annotate("description", "Rate of electron transport")

  val A_j = Double("c:A_j") default(0) nonnegative()
  ...
  val StarchCarbon = Double("c:StarchCarbon") nonnegative()
  val SugarCarbon = Double("c:SugarCarbon") nonnegative()
  val LeafCarbon = Double("c:LeafCarbon") default(1.0) nonnegative() // We start with 2 leaves. Is this a good initial value?
  val RootCarbon = Double("c:RootCarbon") default(1.0) nonnegative()
  ...
  val R_t20 = Double("c:R_t20") nonnegative()
  ...
  val R_below = Double("c:R_below") nonnegative()
  ...
  val TransL = Double("c:TransL") default(0) nonnegative()
  TransL annotate("description", "Translocation from leaf carbon")

  val TransR = Double("c:TransR") default(0) nonnegative()
   ...
   * val RS = Double("c:RS") default(0.12)
   * RS annotate("description", "Root-to-shoot allocation ratio")
   * if (RosetteArea.value < 0.000122) {
   *   RS := 0.12
   * } else if (RosetteArea.value < 0.000396 {
  ...
  val PreviousH = Double("c:PreviousHCD") default(0) param()
  override def step(t: Double, tau: Double) {

    if (PreviousH.value != h.value) {

      CO2i := CO2 * p1
      K_c := K_c25 * Math.exp((H_c * (Temperature - 25)) / (298 * GasConstant * (Temperature + 273)))
      K_o := K_o25 * Math.exp((H_o * (Temperature - 25)) / (298 * GasConstant * (Temperature + 273)))
      V_cmax := V_cmax25 * Math.exp((H_V * (Temperature - 25)) / (298 * GasConstant * (Temperature + 273)))

      Gamma_star := p2 + (p3 * (Temperature - 25)) + (p4 * (Temperature - 25) * (Temperature - 25))
      A_c := V_cmax * ((CO2i - Gamma_star) / (CO2i + (K_c * (1 + (O2i / K_o)))))

      J_max25 := R_JV * V_cmax25
      J_max := J_max25 * Math.exp((H_J * (Temperature - 25)) / (298 * GasConstant * (Temperature + 273))) * (1 + Math.exp(((298 * p5) - p6) / (298 * GasConstant))) / (1 + Math.exp((((Temperature + 273) * p5) - p6) / ((Temperature + 273) * GasConstant)))
      PAR := Light
      J_b := (((PAR * (1 - f_spec)) / 2) + J_max)
      J_c := (((PAR * (1 - f_spec)) / 2) * J_max)
      J_discriminant := Math.sqrt((J_b * J_b) - (4 * p7 * J_c))
      J_1 := ((0 - J_b) + J_discriminant) / (2 * p7)
      J_2 := ((0 - J_b) - J_discriminant) / (2 * p7)
      J := {
        if (J_discriminant.value >= 0) {
          if (J_1.value >= J_2.value) {
            J_1
          } else {
            J_2
          }
      ...
      A_j := (J * (CO2i - Gamma_star)) / (4 * (CO2i - (2 * Gamma_star)))
      ...
      CarbonAssimilation := 12 * 0.000001 * 3600 * A_net * RosetteArea
      ...
      R_t20 := (p8 * (SugarCarbon / RosetteArea)) + p9
      R_t := R_t20 * Math.exp((H_r * (Temperature - 20)) / (293 * GasConstant * (Temperature + 273)))
      R_above := R_t * RosetteArea
      R_below := R_t * (RootCarbon / LeafCarbon)
      Q_trans := SugarCarbon + St - R_above - R_below
      C_avail := Q_trans - (SSU_min * RosetteArea)
      ...
      d_L := GR_max * LeafCarbon / 24
      d_RL := (d_L * alpha) / (1 - alpha)
      d_R := RS * d_L
      d_RR := (d_R * alpha) / (1 - alpha)
      D_C := d_L + d_RL + d_R + d_RR

      if (D_C.value <= Q_C.value) {
        LG := d_L
        R_ml := d_RL
        RG := d_R
        R_mr := d_RR
